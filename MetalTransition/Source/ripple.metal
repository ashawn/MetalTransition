...
vertex VertexOut ripple_vertex(const device float4* vertexArray [[ buffer(0) ]],
                                 ...
                                 )
{
    ...
    return mix(fromImage.sample(textureSampler, input.texCoords + offset),toImage.sample(textureSampler, input.texCoords),smoothstep(0.2, 1.0, sharedUniform.progress));    
}
